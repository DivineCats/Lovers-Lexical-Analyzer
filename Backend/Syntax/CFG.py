...
PRODUCTION_LIST: Dict[int, Tuple[str, List[str]]] = {
    ...
    219: ("<dear_after_id>", ["<typed_tail_dear>"]),
    220: ("<dearest_after_id>", ["(", "<parameter>", ")", "{", "<typed_func_body>", "}"]),
    221: ("<dearest_after_id>", ["<typed_tail_dearest>"]),
    222: ("<rant_after_id>", ["(", "<parameter>", ")", "{", "<typed_func_body>", "}"]),
    223: ("<rant_after_id>", ["<rant_tail>"]),
    224: ("<status_after_id>", ["(", "<parameter>", ")", "{", "<typed_func_body>", "}"]),
    225: ("<status_after_id>", ["<typed_tail_status>"]),
    # const <data-type> <identifier> = <value/expression> ; (factored so LL(1) chooses by data-type)
    10: ("<top_decl>", ["const", "<const_decl>"]),
    404: ("<const_decl>", ["dear", "id", "=", "<assign_rhs_expr>", ";"]),
    405: ("<const_decl>", ["dearest", "id", "=", "<assign_rhs_expr>", ";"]),
    406: ("<const_decl>", ["rant", "id", "=", "<rant_expr>", ";"]),
    407: ("<const_decl>", ["status", "id", "=", "<assign_rhs_expr>", ";"]),

    # A. TYPED (dear / dearest / status) - Merged C++-like: one decl structure, <assign_rhs_expr> init
    14: ("<typed_tail_dear>", ["=", "<assign_rhs_expr>", "<typed_multi>", ";"]),
    15: ("<typed_tail_dear>", ["<typed_multi>", ";"]),
    16: ("<typed_tail_dear>", ["[", "<dear_array_after_lbracket>"]),
    ...
    413: ("<dear_array_dim_tail>", ["<dear_array_assign>", "<typed_multi>", ";"]),
    17: ("<typed_multi>", [",", "id", "<typed_init_opt>", "<typed_multi>"]),
    18: ("<typed_multi>", ["λ"]),
    19: ("<typed_init_opt>", ["=", "<assign_rhs_expr>"]),
    20: ("<typed_init_opt>", ["λ"]),

    # B. TYPED (dearest) - same structure, dearest array branch
    21: ("<typed_tail_dearest>", ["=", "<assign_rhs_expr>", "<typed_multi>", ";"]),
    22: ("<typed_tail_dearest>", ["<typed_multi>", ";"]),
    23: ("<typed_tail_dearest>", ["[", "<dearest_array_after_lbracket>"]),
    ...
    415: ("<dearest_array_dim_tail>", ["<dearest_array_assign>", "<typed_multi>", ";"]),
   ...
   # C. RANT (String) - Uses <rant_expr> (Concat only). Array size = <dear_expr>
    ...
    # D. TYPED (status) - same structure, status array branch
    35: ("<typed_tail_status>", ["=", "<assign_rhs_expr>", "<typed_multi>", ";"]),
    36: ("<typed_tail_status>", ["<typed_multi>", ";"]),
    37: ("<typed_tail_status>", ["[", "<status_array_after_lbracket>"]),
    ...
    419: ("<status_array_dim_tail>", ["<status_array_assign>", "<typed_multi>", ";"]),
    ...
    49: ("<local_decl>", ["dear", "id", "<typed_tail_dear>"]),
    50: ("<local_decl>", ["dearest", "id", "<typed_tail_dearest>"]),
    51: ("<local_decl>", ["rant", "id", "<rant_tail>"]),
    52: ("<local_decl>", ["status", "<status_local_decl>"]),
    441: ("<status_local_decl>", ["id", "<typed_tail_status>"]),
